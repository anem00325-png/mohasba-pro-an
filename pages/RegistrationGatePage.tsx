...
interface RegistrationGatePageProps {
  onRegisterAndLogin: (email: string) => void;
  isOwnerEmail: (email: string) => boolean;
}

const RegistrationGatePage: React.FC<RegistrationGatePageProps> = ({ onRegisterAndLogin, isOwnerEmail }) => {
  const { t } = useLanguage();
          ...
          <div className="text-center space-y-2">
            <div className="flex justify-center items-center space-x-3 rtl:space-x-reverse">
               <svg xmlns="http://www.w3.org/2000/svg" className="h-10 w-10 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                <h1 className="text-3xl font-bold text-white">{t('registration.title')}</h1>
            </div>
            <p className="text-dark-400">{t('registration.subtitle')}</p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="email" className="sr-only">{t('registration.emailLabel')}</label>
              <input
                id="email"
                name="email"
                type="email"
